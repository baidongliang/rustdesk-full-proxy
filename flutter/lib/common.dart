...
bool isInHomePage() {
  final controller = Get.find<DesktopTabController>();
  // Match by key rather than index, so the Home page is still detected
  // correctly when other tabs are inserted before it.
  final state = controller.state.value;
  if (state.tabs.isEmpty) return false;
  return state.selectedTabInfo.key == kTabLabelHomePage;
}
